...
**HabitTrackerWidget**: Tracks a named habit with a daily boolean check-in. Config fields: `habit_name` (text), `streak_goal` (integer, default 7). Stores check-ins as entries with type = 'habit' in the entries table. Renders a 7-day sparkline using `fl_chart`.

**DailyReminderWidget**: Displays a single configurable text reminder, optionally recurring. Config fields: `message` (text), `display_time` (text — HH:mm), `repeat` (enum: daily|weekdays|weekends|none). Widget shows the message and a dismiss button. Dismissed state resets at midnight (stored in widget config, not the database).
...
Allows the actor to upload PDF files (sides, full scripts) and view them in-app. Upload stores the file in Supabase Storage under `{user_id}/scripts/{filename}`. The widget renders a file picker list; selecting a file opens a full-screen overlay using `flutter_pdfview`. Config fields: `default_folder` (text, default "scripts").

PDF viewing on Flutter web uses `flutter_pdfview` which depends on a native PDF renderer. On web, render PDFs by loading the file bytes and passing them to `flutter_pdfview`'s bytes constructor — do not use a URL redirect or iframe. If the file exceeds 20 MB, show a warning before downloading.
...
CREATE TABLE entries (
  ...
  type TEXT NOT NULL CHECK (type IN ('task','event','deadline','habit')),
  title TEXT NOT NULL,
  description TEXT,
  date DATE,
  start_time TIME,
