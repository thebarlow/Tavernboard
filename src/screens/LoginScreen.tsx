...
export default function LoginScreen() {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      // No setIsLoading(false) on success: the page is navigating away to Google.
    } catch (err) {
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="w-full max-w-sm bg-surface rounded-lg border border-divider p-8">
        ...
        <button
          ...
          disabled={isLoading}
          className="w-full border border-divider text-text-primary hover:text-parchment hover:border-parchment disabled:opacity-50 py-2 rounded transition-colors text-sm mb-3 flex items-center justify-center gap-2"
        >
