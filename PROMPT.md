...
Do not:
- Start Phase 2 or any later phase
- Scaffold files for future phases
- Create unused files or stubs

Stop after Phase 1 Verification Gate is satisfied.
...
1. Work on one phase at a time. This session is Phase 1 only.
2. Implement files in this strict dependency order:
   1. Models
   2. Services
   3. Providers
   4. Core app (`main.dart`, `app.dart`, `router.dart`)
   5. Theme
   6. Shared widgets (`lib/widgets/`)
   7. Screens
   8. Dashboard system (`lib/dashboard/`)
   9. Dashboard widgets (`lib/dashboard_widgets/`)
   10. Utilities
   11. SQL / config files
3. After implementing each file, ensure all imports resolve logically — do not leave dangling references.
4. Do not stub methods, leave TODO comments, or return dummy data. Every feature must be fully implemented. If a feature cannot be completed, stop and explain why — **unless** the missing detail is a non-blocking UI/UX specifics (see rule 5 below).
5. **If a required implementation detail is not explicitly defined in this document:** use standard Flutter/Supabase conventions, make the simplest reasonable assumption, and document the assumption in a brief inline comment. Do not stop for non-blocking gaps. Only stop if the gap blocks core functionality (e.g., missing a DB column name or auth flow step).
6. After completing the phase, validate against the Phase Verification Gate and list which criteria are satisfied.
7. If any requirement is explicitly contradicted within this document, stop and ask before proceeding.
8. The `WidgetRegistry` must be initialized exactly once — register all widgets in `main.dart` before `runApp()`. Do not register widgets anywhere else.

---

## Coding Standards

Apply these rules universally. No exceptions.

- Always use Riverpod for state. No `setState` except for ephemeral local UI state (e.g., form field focus).
- Always use `async`/`await`. No `.then()` chains.
- All services must be stateless pure functions or classes with no mutable fields.
- Separate UI from service logic. Widgets call providers; providers call services; services call Supabase.
- No hardcoded colors for primary UI elements. Neutral system values (`Colors.transparent`, `Colors.black54` for drop shadows, `Colors.white` as a tint modifier) are allowed only where they are not part of the tavern palette and using the theme would be semantically wrong.

---

## UI Contract (CRUD Dialogs)

All create/edit dialogs must follow this pattern — no exceptions, no variations between widgets:

- Presented in a `TavernDialog`
- Required fields are labeled with a `*` suffix
- Save and Cancel buttons always present; Save is primary, Cancel is secondary
- Validation: required fields may not be empty; show inline error text below the field on failed submit
- On success: close dialog, call `ref.invalidate(...)` on the relevant provider to refresh
- On error: keep dialog open, show `TavernSnackbar` with a human-readable message (not the raw Supabase error string)

---

## Service Layer Contract

- Dart model field names must match Supabase column names exactly. No transformation layer, no aliasing, no `json_serializable`. Write `fromJson` factories by hand. Dart fields are camelCase; `fromJson` reads from the snake_case key directly. Example pattern — follow this exactly:
...
```
- Service methods map 1:1 to table operations. One method = one Supabase call. No joins inside service methods — compose results in providers if needed.
- Every service method that can fail must propagate the exception. No `try/catch` inside service methods — catching happens in providers.
...
Create `.env` (not committed) by copying `.env.template` and filling in real values. For local development, set `SUPABASE_REDIRECT_URL=http://localhost:8080/auth/callback` — this must also be added as an authorized redirect URI in the Google Cloud Console OAuth app and in the Supabase Auth settings.
...
`String.fromEnvironment` values are compile-time constants — they are **not** read from the environment at runtime. They are injected at build time via `--dart-define-from-file=.env` for local development and via Cloudflare Pages environment variables for production. If you run `flutter run` without `--dart-define-from-file=.env`, these will be empty strings and `Supabase.initialize` will fail silently.
...
You must create exactly these files (in dependency order):

```
lib/main.dart
lib/app.dart
lib/router.dart
lib/theme/tavern_theme.dart
lib/services/supabase_client.dart
...
lib/widgets/tavern_card.dart
lib/widgets/tavern_button.dart
lib/widgets/tavern_text_field.dart
lib/widgets/tavern_dialog.dart
lib/widgets/tavern_snackbar.dart
lib/widgets/loading_overlay.dart
lib/screens/auth/login_screen.dart
lib/screens/auth/auth_callback_screen.dart
lib/dashboard/dashboard_screen.dart
lib/dashboard/dashboard_grid.dart
lib/dashboard/widget_slot.dart
lib/dashboard/widget_registry.dart
lib/dashboard_widgets/base_widget.dart
lib/dashboard_widgets/calendar_widget/calendar_widget.dart
lib/dashboard_widgets/calendar_widget/calendar_widget_config.dart
lib/dashboard_widgets/task_list_widget/task_list_widget.dart
lib/dashboard_widgets/task_list_widget/task_list_widget_config.dart
lib/dashboard_widgets/project_board_widget/project_board_widget.dart
lib/dashboard_widgets/project_board_widget/project_board_widget_config.dart
...
`.env` is created by copying `.env.template` with real local values. It is listed here so the agent creates it — it must be in `.gitignore` and is never committed.

Do not create any file outside this list during Phase 1.
...
`auth_callback_screen.dart`: mounted at `/auth/callback`. Supabase delivers the OAuth token in the **URL fragment** (the `#` portion), not the path or query string. On Flutter Web, `Uri.base` gives the full current URL including the fragment. Pass `Uri.base` to `supabase.auth.getSessionFromUrl(Uri.base)` — do not parse or strip the fragment manually. GoRouter must not intercept or rewrite the fragment — the route `/auth/callback` must match on path only, leaving `Uri.base` intact for Supabase to read. Call `getSessionFromUrl` in `initState` (not in `build`). After session capture, navigate to `/dashboard`. If `getSessionFromUrl` throws or returns no session, show a `TavernSnackbar` with "Sign-in failed — please try again" and navigate back to `/login`. Do not leave the user on a blank or broken screen.
...
Typography: use Google Fonts `Cinzel` for headings and `Lora` for body text via the `google_fonts` package (already in dependencies). Use `GoogleFonts.cinzelTextTheme()` and `GoogleFonts.lora()` — do not bundle font files as assets and do not rely on system fonts. (`MedievalSharp` is not available in the `google_fonts` package; use `Cinzel` instead.)
...
In Phase 1 the dashboard is not yet user-configurable. `WidgetRegistry` and `widget_slot.dart` are scaffolded now so Phase 2 can extend them without touching existing code. In Phase 1, `DashboardGrid` does **not** read from a `DashboardLayout` record — it hardcodes exactly three `WidgetSlot` calls in a `Column`/`Row` depending on screen width. Each `WidgetSlot` receives a hardcoded `widgetTypeKey` string (e.g., `"calendar"`) and an empty config map `{}`. `WidgetSlot` calls `WidgetRegistry.build(typeKey, config)` to resolve the widget — this is the only registry usage in Phase 1. Do not implement any add/remove/drag/configure logic. Render three fixed widget slots in a responsive layout:
...
**Phase 1 scope:** `main.dart` registers exactly three widgets (`"calendar"`, `"task_list"`, `"project_board"`). Dynamic loading from a persisted `DashboardLayout` JSON record is Phase 2. Do not implement that in Phase 1.
...
**Email-to-ID resolution (implement here, in Phase 3):** When an owner invites a user by email, `collaborators.collaborator_id` is NULL at insert time (the invited user may not yet exist in `auth.users`). In Phase 3, add a resolution step to `auth_service.dart` that runs on every successful login, before the dashboard loads: query `collaborators` where `invited_email = currentUser.email AND status = 'pending' AND collaborator_id IS NULL`, and update those rows to set `collaborator_id = currentUser.id`. Without this, RLS policies checking `collaborator_id = auth.uid()` will never match for newly registered invited users. Do not add this logic in Phase 1 or Phase 2.
...
-- token IS NOT NULL prevents returning rows if the column is somehow null-valued
...
11. **Collaborator email-to-ID resolution** *(implement in Phase 3, not before)*: See Phase 3.3 for full instructions. Do not add this logic to `auth_service.dart` in Phase 1 or Phase 2.

12. **Poll vote dedup is intentionally client-only**: Vote deduplication uses `window.localStorage` and is trivially bypassable. This is an accepted MVP tradeoff — do not attempt to add server-side dedup. Do not "fix" this.

13. **`pdfx` on Flutter web requires `canvaskit` renderer**: The `pdfx` package on Flutter web requires the CanvasKit renderer, not the HTML renderer. Ensure the build command uses `--web-renderer canvaskit` (or omit the flag — CanvasKit is the default in Flutter 3.22+). If the HTML renderer is used, `pdfx` will throw an unsupported platform error at runtime.
...
**Phase 1 testing requirement:** Test files must exist and compile. Full mock wiring is not required in Phase 1 — placeholder mocks that satisfy the type system are acceptable. All test files must compile without errors. Full passing test suites are required from Phase 2 onward.
